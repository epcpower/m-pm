...
def load_can_file(
    ...
    for frame in matrix.frames:
        if len(frame.mux_names) == 0:
            ...
            multiplexers = [
                ...
                if isinstance(mux, mpm.canmodel.Multiplexer)
            ]
    ...
    sunspec_root = mpm.sunspecmodel.Root()
...
def build_multiplexed_message(
    ...
    message = message_from_matrix(
        frame=frame,
        factory=mpm.canmodel.MultiplexedMessage,
    )
    ...
    for value, mux_name in sorted(frame.mux_names.items()):
        ...
        multiplexer = mpm.canmodel.Multiplexer(
