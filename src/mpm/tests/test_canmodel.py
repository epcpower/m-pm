    ...
    class Test:
        field = attr.ib(
            metadata=graham.create_metadata(
                field=mpm.canmodel.HexadecimalIntegerField(),
            ),
        )
        none = attr.ib(
            default=None,
...
class SampleModel:
    root = attr.ib(
        factory=lambda: mpm.canmodel.Root(
    ...
    def fill(self):
        self.parameters_message = mpm.canmodel.MultiplexedMessage(
            uuid="e5228583-1e7f-4a8a-a529-bbd84b2d0fca",
        )
        self.root.append_child(self.parameters_message)

        self.parameters_multiplexer = mpm.canmodel.Multiplexer(
...
def test_table_update_unlinked():
    expected_counts = {
        mpm.canmodel.Signal: 2,
        mpm.canmodel.Multiplexer: 24,
    }
