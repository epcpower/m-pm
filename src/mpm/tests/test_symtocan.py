...
def test_other_group_name(hierarchy_file):
    ...
    assert mpm.symtoproject.get_other_name(hierarchy) == other_group_name


def test_load_can_file():
    parameter_root, can_root, sunspec_root = mpm.symtoproject.load_can_path(
    ...
    assert isinstance(can_root, mpm.canmodel.Root)
    assert isinstance(sunspec_root, mpm.sunspecmodel.Root)
...
def test_only_one_parameter_per_query_response_pair(
    ...
    parameter_root, *_ = mpm.symtoproject.load_can_file(
        can_file=sym_file,
        file_type="sym",
