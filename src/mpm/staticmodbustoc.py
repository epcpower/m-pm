...
@builders(mpm.staticmodbusmodel.Root)
@attr.s
class Root:
    wrapped = attr.ib(type=mpm.staticmodbusmodel.Root)
    ...
    def gen(self) -> None:
        ...
        with self.c_path.open("w", newline="\n") as c_file:
            c_file.write(mpm.c.format_nested_lists(c_lines).strip())
            c_file.write("\n")
        ...
        with self.h_path.open("w", newline="\n") as h_file:
            h_file.write(mpm.c.format_nested_lists(h_lines).strip())
            h_file.write("\n")
...
@attr.s
class FunctionData:
    wrapped = attr.ib(type=mpm.staticmodbusmodel.FunctionData)
    parameter_uuid_finder = attr.ib(type=typing.Callable)
    skip_output = attr.ib(default=False, type=bool)
...
@builders(mpm.staticmodbusmodel.FunctionDataBitfield)
@attr.s
class FunctionDataBitfield:
    wrapped = attr.ib(type=mpm.staticmodbusmodel.FunctionDataBitfield)
    parameter_uuid_finder = attr.ib(type=typing.Callable)
    skip_output = attr.ib(default=False, type=bool)
