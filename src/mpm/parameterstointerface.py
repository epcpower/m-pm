...
def find_model_from_point(
    point: typing.Union[mpm.sunspecmodel.DataPoint, mpm.sunspecmodel.DataPointBitfield]
) -> mpm.sunspecmodel.Model:
    """
    ...
    for ancestor in point.ancestors():
        if isinstance(ancestor, mpm.sunspecmodel.Model):
...
def export(
    ...
    template_context = {
        ...
        "interface_items": mpm.c.format_nested_lists(
            built_c,
        ).strip(),
        "declarations": mpm.c.format_nested_lists(built_h).strip(),
    }
...
class Root:
    ...
    def gen(self):
        ...
        def sunspec_node_wanted(node):
            ...
            wanted_types = (
                mpm.sunspecmodel.DataPoint,
                mpm.sunspecmodel.DataPointBitfieldMember,
            )
        ...
        def staticmodbus_node_wanted(node):
            ...
            wanted_types = (
                mpm.staticmodbusmodel.FunctionData,
                mpm.staticmodbusmodel.FunctionDataBitfieldMember,
            )
            if not isinstance(node, wanted_types):
                return False
...
@builders(mpm.staticmodbusmodel.FunctionData)
@attr.s
class FunctionData:
    wrapped = attr.ib()
    parameter_uuid_finder = attr.ib()
...
@builders(mpm.staticmodbusmodel.FunctionDataBitfieldMember)
@attr.s
...
class Parameter:
    ...
    def _local_sunspec_parameter_gen(
        ...
        if sunspec_point is None:
            ...
            hand_coded_getter_function_name = mpm.sunspectointerface.getter_name(
                parameter=parameter,
            ...
            hand_coded_setter_function_name = mpm.sunspectointerface.setter_name(
                parameter=parameter,
                sunspec_id=sunspec_id,
                model_id=model.id,
                is_table=False,
...
class TableBaseStructures:
    ...
    def _find_scale_factor(
        ...
        node_in_model: typing.Union[
            mpm.sunspecmodel.TableRepeatingBlockReferenceDataPointReference,
            mpm.sunspecmodel.DataPoint,
        ],
...
def get_sunspec_model_from_table_group_element(sunspec_point, table_element):
    nodes_in_model = [
        ...
        if isinstance(node, mpm.sunspecmodel.DataPoint)
    ]
    ...
    (model_repeating_block,) = [
        ...
        if isinstance(node, mpm.sunspecmodel.TableRepeatingBlockReference)
    ]
